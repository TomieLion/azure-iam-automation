...
Authenticates securely to Azure using GitHub Secrets.

Executes the deployment scripts in a clean Ubuntu environment.

Validates role assignments automatically.
...
./assign_roles.sh
Cleanup
To avoid unnecessary Azure costs, run the cleanup script to remove all provisioned resources:

Bash
chmod +x cleanup.sh
./cleanup.sh
📂 Project Structure
setup.sh: Provisions the VNet, Subnets, and AD Groups.

assign_roles.sh: Handles the logic for RBAC assignments.

cleanup.sh: Deletes the Resource Group and AD Groups.
